...
namespace Server
{
	public class DestroyableDoorGenerator
	{
		...
		{
			CommandSystem.Register( "DestroyableDoorGen", AccessLevel.Administrator, new CommandEventHandler( DoorGen_OnCommand ) );
		}
		...
		public static BaseDestroyableDoor AddDoor( int x, int y, int z, DoorFacing facing )
		{
			int doorZ = z;
			...
			BaseDestroyableDoor door = new DestroyableDarkWoodDoor( facing );
				...
				for ( int ry = 0; ry < region.Height; ++ry )
				{
					...
					for ( int i = 0; i < tiles.Length; ++i )
					{
						...
						if ( IsWestFrame( id ) )
						{
							...
							{
								BaseDestroyableDoor first = AddDoor( vx + 1, vy, z, DoorFacing.WestCW );
								BaseDestroyableDoor second = AddDoor( vx + 2, vy, z, DoorFacing.EastCCW );
						...
						else if ( IsNorthFrame( id ) )
						{
							...
							{
								BaseDestroyableDoor first = AddDoor( vx, vy + 1, z, DoorFacing.NorthCCW );
								BaseDestroyableDoor second = AddDoor( vx, vy + 2, z, DoorFacing.SouthCW );
